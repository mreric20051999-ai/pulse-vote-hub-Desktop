...
if (!window.location.host) contextBridge.exposeInMainWorld('pvh', {
  ...
  updatePosition: (id, title) => ipcRenderer.invoke('election:position-update', id, title, oid()),
  ...
  updateCandidate: (p) => ipcRenderer.invoke('election:candidate-update', p, oid()),
  ...
  importVotersFile: (eid) => ipcRenderer.invoke('voter:import-file', eid, oid()),
